...
class Machine
	...
	def self.parse_lshw_xml(xmldata)
		...
		begin
		  data[:uuid]					= ckBogus(elems[
															"configuration/setting[@id='uuid']"
														].attributes["value"])
		rescue
			data[:uuid]					= nil
		end
		...
		begin
			data[:chassis]			= ckBogus(elems[
															"configuration/setting[@id='chassis']"
														].attributes["value"])
		rescue
			data[:uuid]					= nil
		end
		...
		##################### MOTHERBOARD #######################
		data[:mobo]						= {}
		data[:mobo][:vendor]	= ckBogus(elems["node[@id='core']/vendor"].text)
		data[:mobo][:product]	= ckBogus(elems["node[@id='core']/product"].text)

		##################### PROCESSOR #########################
		data[:cpu]						= {}
		data[:cpu][:vendor]		= ckBogus(elems[
															"node[@id='core']/node[@id='cpu']/vendor"].text)
		data[:cpu][:product]	= ckBogus(elems[
															"node[@id='core']/node[@id='cpu']/product"].text)
		data[:cpu][:bits]			= ckBogus(elems[
															"node[@id='core']/node[@id='cpu']/width"].text)
		data[:cpu][:clock]		= {}
		data[:cpu][:clock][:value] \
													= ckBogus(elems[
															"node[@id='core']/node[@id='cpu']/clock"].text)
		data[:cpu][:clock][:units] \
													= ckBogus(elems[
															"node[@id='core']/node[@id='cpu']/clock"].
																attributes["units"])
		...
		doc.elements.each("node/node[@id='core']/node[@id='memory']/node") do |slot|
			...
			hash[:units] = slot.elements["size"].attributes["units"]
			data[:ram].push hash
		end
		...
		[
			...
			doc.elements.each(search_pattern) do |disk|
				...
				hash[:size]				= {}
				hash[:size][:units] \
													= disk.elements["size"].attributes["units"]
				hash[:size][:value] \
													= disk.elements["size"].text
				################# PARTITIONS ##########################
